...
void dumper::write_game_assembly() {
...

  {
    const char *gc_exports[] = {
        "il2cpp_gchandle_get_target", "il2cpp_gchandle_new",
        "il2cpp_gchandle_new_weakref", "il2cpp_gchandle_free", nullptr};
    for (int e = 0; gc_exports[e] && !gc_handles; e++) {
      uint8_t *fn =
          (uint8_t *)GetProcAddress((HMODULE)game_base, gc_exports[e]);
      if (!fn)
        continue;
      ...
      for (int hop = 0; hop < 4; hop++) {
        if (target[0] == 0xE9) {
          ...
          // JMP [RIP+X]
          uint8_t **ptr_addr =
              (uint8_t **)(target + 6 + *(int32_t *)(target + 2));
      ...
      for (int i = 0; i < 0x400 && !gc_handles; i++) {
        ...
        if ((target[i] == 0x48 || target[i] == 0x4C) &&
          ...
          //  AND reg, 7 within 40 bytes before or after
          int jlo = i > 40 ? i - 40 : 0;
          for (int j = jlo; j < i + 40 && j < 0x400 && !gc_handles; j++) {
            if (target[j] == 0x83 && (target[j + 1] & 0xF8) == 0xE0 &&
                target[j + 2] == 0x07) {
              uint64_t candidate =
                  DUMPER_RVA((uint64_t)dumper::relative_32(target + i, 3));
              if (candidate) {
                gc_handles = candidate;
                write_to_log(
                    "[GC_HANDLES] Pattern 5 (export %s impl) - RVA: 0x%llx\n",
  ...
  if (!gc_handles) {
    ...
    if (sig) {
      ...
      write_to_log(
          "[GC_HANDLES] Pattern 1 (LEA RAX + AND ECX,7) - RVA: 0x%llx\n",
          gc_handles);
    }
  }
  ...
  // Pattern 4: LEA RBX, [rip+?] then AND any reg, 7 cuz we love cracking with 4
